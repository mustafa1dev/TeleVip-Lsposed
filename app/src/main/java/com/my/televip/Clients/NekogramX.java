...
/**
 * Resolver for Nekogram X ({@code nekox.messenger}).
 *
 * <p>NekoX ships unobfuscated Telegram symbols, exactly like its Momogram fork
 * ({@code nekox.messenger.broken} / {@code momo.gram}), so the mapping tables stay empty and every
 * name resolves to the upstream Telegram name. Only the tgnet parameter signature has to be
 * registered. If a future NekoX build starts shipping R8-renamed symbols, populate the lists below
 * the same way {@link Nekogram} and {@link Cherrygram} do.</p>
 */
public class NekogramX {
    ...
    public static String resolveMethodName(String className, String name) {
        String alias = methodAlias.get(className + "#" + name);
